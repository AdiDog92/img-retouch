...
function Column({ list, status, itemClassName }: ColumnProps) {
	...
	return (
		<section className="flex flex-col gap-2 snap-center h-full">
			<header>
			...
			<GridList
				...
				className="flex-1 min-h-0 overflow-y-auto p-2 md:p-4 overflow-y-auto overflow-x-hidden relative outline outline-0 bg-white/70 dark:bg-zinc-900/60 backdrop-blur-sm border border-black/10 dark:border-white/10 bg-clip-padding text-gray-700 dark:text-zinc-400 flex flex-col gap-3 rounded-xl shadow-xl drop-target:bg-blue-200 dark:drop-target:bg-blue-800/60 drop-target:outline-2 outline-blue-500 forced-colors:outline-[Highlight] -outline-offset-2 empty:items-center empty:justify-center"
			>
